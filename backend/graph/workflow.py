"""
LangGraph Workflow for SunShift
"""
...
from agents.solar_forecast_agent import SolarForecastAgent

logging.basicConfig(level=logging.INFO)
logger = logging.getLogger(__name__)


class SunShiftState(TypedDict):
    """State for the SunShift workflow"""
    raw_data: Dict
    processed_data: Dict
...
class SunShiftWorkflow:
    """LangGraph workflow orchestrator for SunShift"""
    
    def __init__(self):
        self.data_agent = DataAgent()
        self.feature_agent = FeatureAgent()
        # Using SolarForecastAgent now - created in _generate_forecast method
    ...
    def _build_graph(self) -> StateGraph:
        ...
        workflow = StateGraph(SunShiftState)
    ...
    def _collect_data(self, state: SunShiftState) -> SunShiftState:
        """Node: Collect data"""
    ...
    def _process_features(self, state: SunShiftState) -> SunShiftState:
    ...
    def _generate_forecast(self, state: SunShiftState) -> SunShiftState:
        ...
        try:
            ...
            if "train_data" in processed_data:
                # Use SolarForecastAgent
                forecast_agent = SolarForecastAgent()
    ...
    def _generate_insights(self, state: SunShiftState) -> SunShiftState:
        """Node: Generate insights - Enhanced agent already provides insights"""
        logger.info("Workflow: Insights already generated by SolarForecastAgent")
    ...
    def _handle_chat(self, state: SunShiftState) -> SunShiftState:
    ...
    def run_full_pipeline(self, latitude: float = 28.6139, longitude: float = 77.2090) -> Dict:
        ...
        logger.info(f"Starting full SunShift pipeline for ({latitude}, {longitude})...")
        
        initial_state = SunShiftState(
            raw_data={},
            processed_data={},
            forecast_result={},
            insight_result={},
...
# Singleton instance
workflow_instance = SunShiftWorkflow()
